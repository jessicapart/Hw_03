...
gss16_advfront <- linear_reg() %>%
  set_engine("lm") %>%
  fit(emailhr~educ, data = gss16_advfront) 

gss16_advfront_aug <- augment(gss16_advfront$fit)


ggplot(gss16_advfront_aug, mapping = aes(x = .fitted, y = .resid)) +
  geom_point(alpha = 0.5) +
  geom_hline(yintercept = 0, color = "gray", lty = "dashed") +
  labs(x = "Predicted weekly email time", y = "Residuals")
...
```{r}
gss16_rec_1 <- recipe(spam~., data = gss16_train)
