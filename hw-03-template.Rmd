...
Weekly hours on email = 0.6854*education level -2.7573

The estimated value of weekly time spent on email uses a linear regression model of the education
level. The equation used to calculate the estimated emailhr for each level of education uses the
gradient and y-intercept of the line of best fit through data points of emailhr plotted against educ.

b) Comment on the overall performance of the linear regression model. Support your statements with an appropriate data visualization and model fit statistics.

For each additional education qualification the weekly time spent on email is expected to increase by 0.685 hrs,this shows a slight positive correlation between education and email time. One issue with the model is that the y intercept is at -2.79 hours which is impossible for this data set. A graph of the residuals show a fanning effect in the x direction indicating that a linear regression model may be unsuitable.

```{r}
ggplot(data = gss16_advfront, aes(x = educ, y = emailhr)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "Education level", y = "Weekly time on email(hours)", title = "Predicting weekly hours spent on email from education", subtitle = "Linear Regression Model")
...
gss16_advfront_1 <- linear_reg() %>%
  set_engine("lm") %>%
  fit(emailhr~educ, data = gss16_advfront)

gss16_advfront_aug <- augment(gss16_advfront_1$fit)
...
```{r}
gss16_rec_1 <- recipe(advfront~educ, data = gss16_split)

gss16_mod_1 <- logistic_reg() %>%
  set_engine("glm")

gss16_rec_1 <- gss16_rec_1 %>%
...
I have chosen to use a logistic regression model. Seeing how the responses are bound between 'Strongly disagree' and 'Strongly Agree' the x values should tend to infinity at bound values of the response. I think that it's likely a higher level of education would lead to a positive response to the posed question and so I have used the positive version of this model.
...
```{r}
gss16_rec_2 <- recipe(advfront~polviews + wrkstat + educ, data = gss16_split)

gss16_mod_2 <- logistic_reg() %>%
  set_engine("glm")

gss16_rec_2 <- gss16_rec_2 %>%
  step_dummy(all_nominal(), -all_outcomes()) 
...
Model 1 performs better. The ROC curve for the second model is close to a straight line which indicates that the model holds very little predictive power wheres the curve for the first model is more obvious and has a greater area above the random line 
